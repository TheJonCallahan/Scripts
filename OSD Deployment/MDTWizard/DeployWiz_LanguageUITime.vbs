...
'''''''''''''''''''''''''''''''''''''
'  Custom data and time functions
'
'''''''''''''''''''''''''''''''''''''

Function DateTime_Initialization
	
	SelectMonth.value = Month(date)
	SelectDay.value = Day(date)
	SelectYear.value = Year(date)
	SelectTime.value = Time()
	
	ApplyDateTime.disabled = true
...
Function SetDateTime
	...
	dim sCMD
	dim sSysTime
	dim sSysMonthDateYear
	
	sSysMonthDateYear = FormatDateTime(SelectMonth.value & "/" & SelectDay.value & "/" & SelectYear.value,2)
	
	sSysTime = FormatDateTime(SelectTime.value,3)
	
	sCmd = "cmd /c date " & sSysMonthDateYear
	TestAndLog oUtility.RunWithHeartbeat(sCmd), "Updating system date to " & sCmd
	
	sCmd = "cmd /c time " & sSysTime
	TestAndLog oUtility.RunWithHeartbeat(sCmd), "Updating system time to " & sCmd
	
	SetDateTimeSuccess.style.display = "inline"
	SetDateTimeSuccess.innertext = "Date and time successfully updated to " & sSysMonthDateYear & " " & sSysTime
	
	ButtonNext.disabled = FALSE

End Function

Function DateTime_Validation

	Date_err.style.display = "none"
	Time_err.style.display = "none"

	Dim isValidDate, isValidTime
	
	isValidDate = IsDate(SelectMonth.value & "/" & SelectDay.value & "/" & SelectYear.value)
	isValidTime = IsDate(SelectTime.value)
	
	If isValidDate = FALSE Then
	
		Date_err.style.display = "inline"
	
	End If
	
	If isValidTime = FALSE Then
	
		Time_err.style.display = "inline"
	
	End If
	
	If ((isValidDate = TRUE) AND (isValidTime = TRUE)) Then
	
		ApplyDateTime.disabled = FALSE
